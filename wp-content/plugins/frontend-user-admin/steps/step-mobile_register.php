...
<p><label for="ms_domain"><?php _e('Site Domain', 'frontend-user-admin') ?></label> <span class="required"><?php echo $required; ?></span><br />
<input type="text" name="ms_domain" id="ms_domain" class="input frontend-user-admin-ms_domain" value="<?php if ( !empty($_POST['ms_domain']) ) echo esc_attr($_POST['ms_domain']); ?>" /> <?php _e('Half-width alphamerics', 'frontend-user-admin') ?></p>
<?php endif; ?>
<?php		break;
			case "ms_title": ?>
<?php if( is_multisite() && !empty($options['global_settings']['register_ms_title']) ) : ?>
<p><label for="ms_title"><?php _e('Site Title', 'frontend-user-admin') ?></label> <span class="required"><?php echo $required; ?></span><br />
<input type="text" name="ms_title" id="ms_title" class="input frontend-user-admin-ms_title" value="<?php if ( !empty($_POST['ms_title']) ) echo esc_attr($_POST['ms_title']); ?>" /></p>
<?php endif; ?>
<?php		break;
			case "user_login": ?>
<?php if( empty($options['global_settings']['email_as_userlogin']) && !empty($options['global_settings']["register_user_login"]) ) : ?>
<p><label for="user_login"><?php _e('Username', 'frontend-user-admin') ?></label> <span class="required"><?php echo $required; ?></span><br />
<input type="text" name="user_login" id="user_login" class="input frontend-user-admin-user_login" value="<?php if ( isset($_POST['user_login']) ) echo esc_attr($_POST['user_login']); ?>" size="20" /> <?php _e('Half-width alphamerics', 'frontend-user-admin') ?></p>
<?php endif; ?>
<?php		break;
			case "last_name": ?>
<?php if( !empty($options['global_settings']["register_last_name"]) ) : ?>
<p><label for="last_name"><?php _e('Last name', 'frontend-user-admin') ?></label><?php if ( !empty($options['global_settings']["register_last_name_required"]) ) echo ' <span class="required">'.$required.'</span>'; ?><br />
<input type="text" name="last_name" id="last_name" class="input last_name" value="<?php if ( isset($_POST['last_name']) ) echo esc_attr($_POST['last_name']); ?>" /></p>
<?php endif; ?>
<?php		break;
			case "first_name": ?>
<?php if( !empty($options['global_settings']["register_first_name"]) ) : ?>
<p><label for="first_name"><?php _e('First name', 'frontend-user-admin') ?></label><?php if ( !empty($options['global_settings']["register_first_name_required"]) ) echo ' <span class="required">'.$required.'</span>'; ?><br />
<input type="text" name="first_name" id="first_name" class="input first_name" value="<?php if ( isset($_POST['first_name']) ) echo esc_attr($_POST['first_name']); ?>" /></p>
<?php endif; ?>
<?php		break;
			case "nickname": ?>
<?php if( !empty($options['global_settings']["register_nickname"]) ) : ?>
<p><label for="nickname"><?php _e('Nickname', 'frontend-user-admin') ?></label><?php if ( !empty($options['global_settings']["register_nickname_required"]) ) echo ' <span class="required">'.$required.'</span>'; ?><br />
<input type="text" name="nickname" id="nickname" class="input nickname" value="<?php if ( isset($_POST['nickname']) ) echo esc_attr($_POST['nickname']); ?>" /></p>
<?php endif; ?>
<?php		break;
			case "user_email": ?>
<?php if( !empty($options['global_settings']["register_user_email"]) ) : ?>
<p><label for="user_email"><?php _e('E-mail', 'frontend-user-admin') ?></label> <span class="required"><?php echo $required; ?></span><br />
<input type="text" name="user_email" id="<?php if ( !empty($options['global_settings']['email_as_userlogin']) ) echo 'user_login'; else echo 'user_email'; ?>" class="input user_email" value="<?php if ( isset($_POST['user_email']) ) echo esc_attr($_POST['user_email']); ?>" size="25"<?php if ( !empty($options['global_settings']['email_confirmation_first']) ) echo ' readonly="readonly"'; ?> /></p>
<?php endif; ?>
<?php		break;
			case "user_url": ?>
<?php if( !empty($options['global_settings']["register_user_url"]) ) : ?>
<p><label for="user_url"><?php _e('Website', 'frontend-user-admin') ?></label><?php if ( !empty($options['global_settings']["register_user_url_required"]) ) echo ' <span class="required">'.$required.'</span>'; ?><br />
<input type="text" name="user_url" id="user_url" class="input user_url" value="<?php if ( isset($_POST['user_url']) ) echo esc_attr($_POST['user_url']); ?>" /></p>
<?php endif; ?>
<?php		break;
			case "aim": ?>
<?php if( !empty($options['global_settings']["register_aim"]) ) : ?>
<p><label for="aim"><?php _e('AIM', 'frontend-user-admin') ?></label><?php if ( !empty($options['global_settings']["register_aim_required"]) ) echo ' <span class="required">'.$required.'</span>'; ?><br />
<input type="text" name="aim" id="aim" class="input aim" value="<?php if ( isset($_POST['aim']) ) echo esc_attr($_POST['aim']); ?>" /></p>
<?php endif; ?>
<?php		break;
			case "yim": ?>
<?php if( !empty($options['global_settings']["register_yim"]) ) : ?>
<p><label for="yim"><?php _e('Yahoo IM', 'frontend-user-admin') ?></label><?php if ( !empty($options['global_settings']["register_yim_required"]) ) echo ' <span class="required">'.$required.'</span>'; ?><br />
<input type="text" name="yim" id="yim" class="input yim" value="<?php if ( isset($_POST['yim']) ) echo esc_attr($_POST['yim']); ?>" /></p>
<?php endif; ?>
<?php		break;
			case "jabber": ?>
<?php if( !empty($options['global_settings']["register_jabber"]) ) : ?>
<p><label for="jabber"><?php _e('Jabber / Google Talk', 'frontend-user-admin') ?></label><?php if ( !empty($options['global_settings']["register_jabber_required"]) ) echo ' <span class="required">'.$required.'</span>'; ?><br />
<input type="text" name="jabber" id="jabber" class="input jabber" value="<?php if ( isset($_POST['jabber']) ) echo esc_attr($_POST['jabber']); ?>" /></p>
<?php endif; ?>
<?php		break;
			case "description": ?>
<?php if( !empty($options['global_settings']["register_description"]) ) : ?>
<p><label for="description"><?php _e('Biographical Info', 'frontend-user-admin'); ?></label><?php if ( !empty($options['global_settings']["register_description_required"]) ) echo ' <span class="required">'.$required.'</span>'; ?><br />
<textarea name="description" id="description" class="textarea description" rows="5" cols="30"><?php if ( isset($_POST['description']) ) echo esc_attr($_POST['description']) ?></textarea><br /><?php _e('Share a little biographical information to fill out your profile. This may be shown publicly.', 'frontend-user-admin'); ?></p>
<?php endif; ?>
<?php		break;
			case "role":
			break;
...
?>
<p><label for="pass1"><?php _e('Password', 'frontend-user-admin'); ?></label> <span class="required"><?php echo $required; ?></span><br />
...
<p><?php _e('Password Strength', 'frontend-user-admin'); ?></p>
<div id="pass-strength-result"><?php _e('Strength indicator', 'frontend-user-admin'); ?></div>
<p><?php _e('Hint: Use upper and lower case characters, numbers and symbols like !"?$%^&amp;( in your password.', 'frontend-user-admin'); ?></p>
<?php endif; ?>
<?php endif; ?>
<?php endif; ?>
<?php		break;
...
<p><label for="<?php echo $val; ?>"><?php echo $options['user_attribute']['user_attribute'][$i]['label']; ?></label><?php if($options['user_attribute']['user_attribute'][$i]['required']) echo ' <span class="required">'.$required.'</span>'; ?><br />
...
<input type="text" name="<?php echo $val; ?>" id="<?php echo $val; ?>" value="<?php if( isset($_POST[$val]) ) : echo esc_attr($_POST[$val]); else : echo esc_attr($options['user_attribute']['user_attribute'][$i]['default']); endif; ?>" class="input <?php echo $val; ?>"<?php if ( $val=='zipcode' && $net_shop_admin ) : if ( !function_exists('is_ktai') || function_exists('is_ktai') && !is_ktai()) : ?> onKeyUp="AjaxZip2.zip2addr(this,'pref','address1');"<?php endif; endif; ?><?php echo $readonly; ?><?php echo $disabled; ?> /> 
<?php							break;
							case 'textarea': ?>
<textarea name="<?php echo esc_attr($val); ?>" id="<?php echo $val; ?>" class="textarea <?php echo $val; ?>"<?php echo $readonly; ?><?php echo $disabled; ?>><?php if( isset($_POST[$val]) ) : echo htmlspecialchars($_POST[$val]); else : echo htmlspecialchars($options['user_attribute']['user_attribute'][$i]['default']); endif; ?></textarea>
<?php						break;
							case 'select':
...
<input type="checkbox" name="<?php echo $val; ?>[]" class="checkbox <?php echo $val; ?>" value="<?php echo esc_attr($value); ?>"<?php if((!empty($_POST[$val]) && is_array($_POST[$val]) && in_array($value, $_POST[$val])) || !empty($default) ) echo ' checked="checked"';?><?php echo $disabled; ?> /> <?php echo $label; ?> 
<?php							endforeach; ?>
<?php						break;                                                                          
							case 'radio':
								preg_match_all('/"[^"]*"d?|\'[^\']*\'d?/', $options['user_attribute']['user_attribute'][$i]['default'], $matches);
...
<input type="radio" name="<?php echo $val; ?>" class="radio <?php echo $val; ?>" value="<?php echo esc_attr($value); ?>"<?php if( (isset($_POST[$val]) && $value == $_POST[$val]) || isset($default)) echo ' checked="checked"';?><?php echo $disabled; ?> /> <?php echo $label; ?> 
<?php							endforeach; ?>
<?php						break;
							case 'datetime':
...
<p><label for="terms_of_use"><?php _e('Terms of use', 'frontend-user-admin') ?></label><?php echo ' <span class="required">'.$required.'</span>'; ?><br />
<input type="checkbox" name="terms_of_use" id="terms_of_use" value="1"  <?php if ( !empty($_POST['terms_of_use']) ) checked(1); ?> /> 
