...
<p><label for="ms_domain"><?php _e('Site Domain', 'frontend-user-admin') ?></label><br />
<input type="hidden" name="ms_domain" value="<?php echo esc_attr($_POST['ms_domain']); ?>" /><?php echo esc_attr($_POST['ms_domain']); ?></p>
<?php endif; ?>
<?php		break;
			case "ms_title": ?>
<?php if( is_multisite() && !empty($options['global_settings']['register_ms_title']) ) : ?>
<p><label for="ms_title"><?php _e('Site Title', 'frontend-user-admin') ?></label><br />
<input type="hidden" name="ms_title" value="<?php echo esc_attr($_POST['ms_title']); ?>" /><?php echo esc_attr($_POST['ms_title']); ?></p>
<?php endif; ?>
<?php		break;
			case "user_login": ?>
<?php if( empty($options['global_settings']['email_as_userlogin']) && !empty($options['global_settings']["register_user_login"]) ) : ?>
<p><label for="user_login"><?php _e('Username', 'frontend-user-admin') ?></label><br />
<input type="hidden" name="user_login" value="<?php echo esc_attr($_POST['user_login']); ?>" /><?php echo esc_attr($_POST['user_login']); ?></p>
<?php endif; ?>
<?php		break;
			case "last_name": ?>
<?php if( !empty($options['global_settings']["register_last_name"]) ) : ?>
<p><label for="last_name"><?php _e('Last name', 'frontend-user-admin') ?></label><br />
<input type="hidden" name="last_name" value="<?php echo esc_attr($_POST['last_name']); ?>" /><?php echo esc_attr($_POST['last_name']); ?></p>
<?php endif; ?>
<?php		break;
			case "first_name": ?>
<?php if( !empty($options['global_settings']["register_first_name"]) ) : ?>
<p><label for="first_name"><?php _e('First name', 'frontend-user-admin') ?></label><br />
<input type="hidden" name="first_name" value="<?php echo esc_attr($_POST['first_name']); ?>" /><?php echo esc_attr($_POST['first_name']); ?></p>
<?php endif; ?>
<?php		break;
			case "nickname": ?>
<?php if( !empty($options['global_settings']["register_nickname"]) ) : ?>
<p><label for="nickname"><?php _e('Nickname', 'frontend-user-admin') ?></label><br />
<input type="hidden" name="nickname" value="<?php echo esc_attr($_POST['nickname']); ?>" /><?php echo esc_attr($_POST['nickname']); ?></p>
<?php endif; ?>
<?php		break;
			case "user_email": ?>
<?php if( !empty($options['global_settings']["register_user_email"]) ) : ?>
<p><label for="user_email"><?php _e('E-mail', 'frontend-user-admin') ?></label><br />
<input type="hidden" name="user_email" value="<?php echo esc_attr($_POST['user_email']); ?>" /><?php echo esc_attr($_POST['user_email']); ?></p>
<?php endif; ?>
<?php		break;
			case "user_url": ?>
<?php if( !empty($options['global_settings']["register_user_url"]) ) : ?>
<p><label for="user_url"><?php _e('Website', 'frontend-user-admin') ?></label><br />
<input type="hidden" name="user_url" value="<?php echo esc_attr($_POST['user_url']); ?>" /><?php echo esc_attr($_POST['user_url']); ?></p>
<?php endif; ?>
<?php		break;
			case "aim": ?>
<?php if( !empty($options['global_settings']["register_aim"]) ) : ?>
<p><label for="aim"><?php _e('AIM', 'frontend-user-admin') ?></label><br />
<input type="hidden" name="aim" value="<?php echo esc_attr($_POST['aim']); ?>" /><?php echo esc_attr($_POST['aim']); ?></p>
<?php endif; ?>
<?php		break;
			case "yim": ?>
<?php if( !empty($options['global_settings']["register_yim"]) ) : ?>
<p><label for="yim"><?php _e('Yahoo IM', 'frontend-user-admin') ?></label><br />
<input type="hidden" name="yim" value="<?php echo esc_attr($_POST['yim']); ?>" /><?php echo esc_attr($_POST['yim']); ?></p>
<?php endif; ?>
<?php		break;
			case "jabber": ?>
<?php if( !empty($options['global_settings']["register_jabber"]) ) : ?>
<p><label for="jabber"><?php _e('Jabber / Google Talk', 'frontend-user-admin') ?></label><br />
<input type="hidden" name="jabber" value="<?php echo esc_attr($_POST['jabber']); ?>" /><?php echo esc_attr($_POST['jabber']); ?></p>
<?php endif; ?>
<?php		break;
			case "description": ?>
<?php if( !empty($options['global_settings']["register_description"]) ) : ?>
<p><label for="description"><?php _e('Biographical Info', 'frontend-user-admin'); ?></label><br />
<input type="hidden" name="description" value="<?php echo esc_attr($_POST['description']); ?>" /><?php echo esc_attr($_POST['description']); ?></p>
<?php endif; ?>
<?php		break;
			case "role":
			break;
...
?>
<p><label for="pass1"><?php _e('Password', 'frontend-user-admin'); ?></label><br />

<input type="hidden" name="pass1" value="<?php echo esc_attr($_POST['pass1']); ?>" />
<input type="hidden" name="pass2" value="<?php echo esc_attr($_POST['pass2']); ?>" /><?php echo preg_replace('/./','*',$_POST['pass1']); ?></p>
<?php endif; ?>
<?php endif; ?>
<?php		break;
			default:
...
<p><label for="<?php echo $val; ?>"><?php echo $options['user_attribute']['user_attribute'][$i]['label']; ?></label><br />
