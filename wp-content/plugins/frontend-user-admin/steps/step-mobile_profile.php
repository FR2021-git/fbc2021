...
<input type="text" name="user_login" id="user_login" class="input frontend-user-admin-user_login" value="<?php echo $profileuser->user_login; ?>" disabled="disabled" /><br /><?php _e('Your username cannot be changed', 'frontend-user-admin'); ?></p>
<?php endif; ?>
<?php		break;
			case "last_name": ?>
<?php if( !empty($options['global_settings']["profile_last_name"]) && empty($options['global_settings']["profile_last_name_admin"]) ) : ?>
<p><label for="last_name"><?php _e('Last name', 'frontend-user-admin') ?></label><?php if ( !empty($options['global_settings']["profile_last_name_required"]) ) echo ' <span class="required">'.$required.'</span>'; ?><br />
<input type="text" name="last_name" id="last_name" class="input frontend-user-admin-last_name" value="<?php echo $profileuser->last_name ?>" /></p>
<?php endif; ?>
<?php		break;
			case "first_name": ?>
<?php if( !empty($options['global_settings']["profile_first_name"]) && empty($options['global_settings']["profile_first_name_admin"]) ) : ?>
<p><label for="first_name"><?php _e('First name', 'frontend-user-admin') ?></label><?php if ( !empty($options['global_settings']["profile_first_name_required"]) ) echo ' <span class="required">'.$required.'</span>'; ?><br />
<input type="text" name="first_name" id="first_name" class="input frontend-user-admin-first_name" value="<?php echo $profileuser->first_name ?>" /></p>
<?php endif; ?>
<?php		break;
			case "nickname": ?>
<?php if( !empty($options['global_settings']["profile_nickname"]) && empty($options['global_settings']["profile_nickname_admin"]) ) : ?>
<p><label for="nickname"><?php _e('Nickname', 'frontend-user-admin') ?></label><?php if ( !empty($options['global_settings']["profile_nickname_required"]) ) echo ' <span class="required">'.$required.'</span>'; ?><br />
<input type="text" name="nickname" id="nickname" class="input frontend-user-admin-nickname" value="<?php echo $profileuser->nickname ?>" /></p>
<?php endif; ?>
<?php		break;
			case "display_name": ?>
<?php if( !empty($options['global_settings']["profile_display_name"]) && empty($options['global_settings']["profile_display_name_admin"]) ) : ?>
<p><label for="display_name"><?php _e('Display name publicly&nbsp;as', 'frontend-user-admin') ?></label><?php if ( !empty($options['global_settings']["profile_display_name_required"]) ) echo ' <span class="required">'.$required.'</span>'; ?><br />
...
<input type="text" name="user_email" id="<?php if ( !empty($options['global_settings']['email_as_userlogin']) ) echo 'user_login'; else echo 'user_email'; ?>" class="input user_email frontend-user-admin-user_email" value="<?php echo $profileuser->user_email ?>" />
...
<p><label for="user_url"><?php _e('Website', 'frontend-user-admin') ?></label><?php if ( !empty($options['global_settings']["profile_user_url_required"]) ) echo ' <span class="required">'.$required.'</span>'; ?><br />
<input type="text" name="user_url" id="user_url" class="input user-url frontend-user-admin-user_url" value="<?php echo $profileuser->user_url ?>" /></p>
<?php endif; ?>
<?php		break;
			case "aim": ?>
<?php if( !empty($options['global_settings']["profile_aim"]) && empty($options['global_settings']["profile_aim_admin"]) ) : ?>
<p><label for="aim"><?php _e('AIM', 'frontend-user-admin') ?></label><?php if ( !empty($options['global_settings']["profile_aim_required"]) ) echo ' <span class="required">'.$required.'</span>'; ?><br />
<input type="text" name="aim" id="aim" class="input frontend-user-admin-aim" value="<?php echo $profileuser->aim ?>" /></p>
<?php endif; ?>
<?php		break;
			case "yim": ?>
<?php if( !empty($options['global_settings']["profile_yim"]) && empty($options['global_settings']["profile_yim_admin"]) ) : ?>
<p><label for="yim"><?php _e('Yahoo IM', 'frontend-user-admin') ?></label><?php if ( !empty($options['global_settings']["profile_yim_required"]) ) echo ' <span class="required">'.$required.'</span>'; ?><br />
<input type="text" name="yim" id="yim" class="input frontend-user-admin-yim" value="<?php echo $profileuser->yim ?>" /></p>
<?php endif; ?>
<?php		break;
			case "jabber": ?>
<?php if( !empty($options['global_settings']["profile_jabber"]) && empty($options['global_settings']["profile_jabber_admin"]) ) : ?>
<p><label for="jabber"><?php _e('Jabber / Google Talk', 'frontend-user-admin') ?></label><?php if ( !empty($options['global_settings']["profile_jabber_required"]) ) echo ' <span class="required">'.$required.'</span>'; ?><br />
<input type="text" name="jabber" id="jabber" class="input frontend-user-admin-jabber" value="<?php echo $profileuser->jabber ?>" /></p>
<?php endif; ?>
<?php		break;
			case "description": ?>
<?php if( !empty($options['global_settings']["profile_description"]) && empty($options['global_settings']["profile_description_admin"]) ) : ?>
<p><label for="description"><?php _e('Biographical Info', 'frontend-user-admin'); ?></label><?php if ( !empty($options['global_settings']["profile_description_required"]) ) echo ' <span class="required">'.$required.'</span>'; ?><br />
<textarea name="description" id="description" class="textarea frontend-user-admin-description" rows="5" cols="30"><?php echo $profileuser->description ?></textarea><br /><?php _e('Share a little biographical information to fill out your profile. This may be shown publicly.', 'frontend-user-admin'); ?></p>
<?php endif; ?>
<?php		break;
			case "role":
			break;
...
?>
<p><label for="pass1"><?php _e('New Password', 'frontend-user-admin'); ?></label><br />
<input type="password" autocomplete="off" name="pass1" id="pass1" class="input frontend-user-admin-pass1" size="16" value="" /><br /><?php _e("If you would like to change the password type a new one. Otherwise leave this blank.", 'frontend-user-admin'); ?><br />
<input type="password" autocomplete="off" name="pass2" id="pass2" class="input frontend-user-admin-pass2" size="16" value="" /><br /><?php _e("Type your new password again.", 'frontend-user-admin'); ?></p>
<?php if( !empty($options['global_settings']["use_password_strength"]) ) : if ( function_exists('is_ktai') && is_ktai() ) break; ?>
<p><?php _e('Password Strength', 'frontend-user-admin'); ?><br />
...
<?php					break;
						case 'textarea': ?>
<textarea name="<?php echo esc_attr($val); ?>" id="<?php echo $val; ?>" class="textarea <?php echo $val; ?>"<?php echo $readonly; ?><?php echo $disabled; ?>><?php if( isset($profileuser->{$val}) ) : echo htmlspecialchars($profileuser->{$val}); else : echo htmlspecialchars($options['user_attribute']['user_attribute'][$i]['default']); endif; ?></textarea>
<?php					break;
						case 'select':
...
<input type="checkbox" name="<?php echo $val; ?>[]" class="checkbox <?php echo $val; ?>" value="<?php echo esc_attr($value); ?>"<?php if((!empty($profileuser->{$val}) && is_array($profileuser->{$val}) && in_array($value, $profileuser->{$val})) || !empty($default) ) echo ' checked="checked"';?><?php echo $disabled; ?> /> <?php echo $label; ?> 
<?php						endforeach; ?>
<?php					break;                                                                          
						case 'radio':
							preg_match_all('/"[^"]*"d?|\'[^\']*\'d?/', $options['user_attribute']['user_attribute'][$i]['default'], $matches);
...
<input type="radio" name="<?php echo $val; ?>" class="radio <?php echo $val; ?>" value="<?php echo esc_attr($value); ?>"<?php if( (isset($profileuser->{$val}) && $value == $profileuser->{$val}) || isset($default) ) echo ' checked="checked"';?><?php echo $disabled; ?> /> <?php echo $label; ?> 
<?php						endforeach; ?>
<?php					break;
						case 'datetime':
...
?>
<p><label for="update_datetime"><?php _e('Update Datetime', 'frontend-user-admin'); ?></label><br />
<?php if ( !empty($profileuser->update_datetime) ) echo date_i18n('Y-m-d H:i:s', $profileuser->update_datetime); ?></p>
<?php
	endif;
?>
